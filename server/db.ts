...
import { InsertUser, users, emailVerifications, InsertEmailVerification, bookings, InsertBooking } from "../drizzle/schema";
...
export async function markEmailVerified(id: number) {
  const db = await getDb();
  if (!db) {
    throw new Error("Database not available");
  }
...
export async function updateBookingEmailVerified(id: number) {
  const db = await getDb();
  if (!db) {
    throw new Error("Database not available");
  }
